...
// The FeeDurationFunc type is an adapter to allow the use of ordinary
// function as FeeDuration mutator.
type FeeDurationFunc func(context.Context, *ent.FeeDurationMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f FeeDurationFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.FeeDurationMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.FeeDurationMutation", m)
	}
	return f(ctx, mv)
}
